...
namespace event_system {
    ...
    public:
         ...
         * @note Non const references can't be used as they cannot be bound to rvalues.
         */
        template<typename... Args, typename Function>
        size_t AddCallback(Function&& callback) {
            return GetOrCreateHandler<Args...>().AddCallback(std::forward<Function>(callback));
        }
        ...
        void Dispatch(Args... args) {
            auto handler = GetHandler<Args...>();
            if (!handler) {
                std::clog << "No handler for event type: " << typeid(EventHandler<Args...>).name() << std::endl;
                return;
            }
